...
async function loadCharts() {
  ...
  try {
    ...
    const chartFontColor = '#7A857F'; // --text-muted
    const chartGridColor = 'rgba(255, 255, 255, 0.05)'; // Borde muy sutil

    // Gradiente dinámico para la curva de tendencia semanal
    const ctx = weeklyCanvas.getContext('2d');
    const gradient = ctx.createLinearGradient(0, 0, 0, 250);
    gradient.addColorStop(0, 'rgba(61, 88, 67, 0.35)'); // --primary con opacidad
    gradient.addColorStop(1, 'rgba(61, 88, 67, 0)');
    ...
    weeklyChartInstance = new Chart(weeklyCanvas, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Alertas',
          data: weeklyCounts,
          borderColor: '#3D5843', // --primary
          backgroundColor: gradient,
          fill: true,
          tension: 0.3, // Suaviza la curva de la línea
          borderWidth: 3,
          pointBackgroundColor: '#60B7BA', // --accent-teal
          pointBorderColor: '#60B7BA',
          pointHoverBackgroundColor: '#60B7BA',
          pointHoverBorderColor: '#60B7BA',
          pointHoverRadius: 6
        }]
      },
      options: {
        ...
        <tr>
          <td colspan="6" class="text-center" style="padding: 40px var(--spacing-xl); color: var(--text-muted);">
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: var(--spacing-sm);">
              ...
              <div style="font-size: 13px; max-width: 300px; color: var(--text-muted);">No hay reportes de residuos pendientes para hoy en Vicente López.</div>
            </div>
          </td>
        </tr>
      `;
